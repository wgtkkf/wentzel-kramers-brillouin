...
# gap
number = 500 # [-], integral parameter, initial: 500 > 10000
nzmax = 1000 # [-], integral parameter, initial: 1000 > 10000

small = 0.01*ucnano # zero [m]
gapmin = small # [m]
gap = gapmin # [m]
gapmax = 1.01*ucnano-small # [m]
...
volt = 0.8 # [V], load (vias) voltage

se = 1 # [eV/V], single electron
volt = se*volt # [eV]

# temperature
temperature_L = 470 # [K], emitter
...
# area
diagonal = 210*ucnano # [m]
Atip = np.pi*np.power(diagonal/np.sqrt(2),2.0) # [m2]
#print(str(Atip))
...
while Ez < Ezmax: # [eV]
    ...
    def JL(arg_Wmax): # emitter, Eq (3.3), arg_Wmax: [eV]
        ...
        #term2 = np.exp(-(arg_Wmax-volt)*ucev_to_j/(kb*temperature_L)) #[J/J]
    ...
    def JR(arg_Wmax): # collector, Eq (3.3), arg_Wmax: [eV]
        ...
        #term2 = np.exp((-(arg_Wmax)*ucev_to_j)/(kb*temperature_R)) # [J/J]
        term3 = cA*term1*term2
    ...
    def NR(arg_Ez): # [eV] collector
        ...
        #term2 = -(arg_Ez-volt-Ef_R)*ucev_to_j/(kb*temperature_R) # [-] eV to J
        integrand = term1*np.log(1+np.exp(term2)) # P.35, Devon's Ph.D. thesis
...
# output
f4.write('nm V W/cm2 W/cm2 A/cm2 A/cm2 nW/K nW/K W/m2K W/m2K')
f4.write('\n')
...
f4.write(str(' '))
f4.write(str(cond_qe/ucnano)) # [nW/K], electron tunneling
f4.write(str(' '))
f4.write(str(cond_therm/ucnano)) # [nW/K], thermionic
f4.write(str(' '))
f4.write(str(cond_qe/Atip)) # [W/m2K], electron tunneling
f4.write(str(' '))
f4.write(str(cond_therm/Atip)) # [W/m2K], thermionic
f4.write('\n')
